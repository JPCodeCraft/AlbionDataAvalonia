...
public class NewSiegeBannerItemEventHandler : EventPacketHandler<NewSiegeBannerItemEvent>
{
    ...
    public NewSiegeBannerItemEventHandler(
        ...
        PlayerState playerState) : base((int)EventCodes.NewSiegeBannerItem)
    {
    ...
    protected override Task OnActionAsync(NewSiegeBannerItemEvent value)
    {
        if (value.Item is not null)
        {
            ...
            if (value.Item.EstimatedMarketValue > 0)
            {
                ...
                if (serverId is null)
                {
                    Log.Debug("Skipping siege banner item estimated market value update because server is not set. ItemUniqueName: {ItemUniqueName}. Quality: {Quality}. Emv: {Emv}.", value.Item.ItemUniqueName, value.Item.Quality, value.Item.EstimatedMarketValue);
                }
                else
                {
